...
class TumUrun extends StatefulWidget {
  @override
  List<dynamic> sel = [];
  List<dynamic>? liste = [];
  String? field;
  ...
  State<TumUrun> createState() => _TumUrunState(
      sel: sel,
      liste: liste,
      appbar: appbar,
      field: field,
...
class _TumUrunState extends State<TumUrun> {
  int _counter = 0;
  List<dynamic> sel;
  List<dynamic>? liste;
  Widget appbar;
  ...
  Future getString(String f, String v) async {
    ...
    sel = await Api().getFilterProduct(field!, value!, search!);

    // sel = await Api().getProductList();
    //print(sel.length);
    //print(sel);
